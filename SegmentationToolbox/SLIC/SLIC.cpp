...
SLIC::~SLIC()
{
	if(m_lvec) mxFree(m_lvec);
	if(m_avec) mxFree(m_avec);
	if(m_bvec) mxFree(m_bvec);


	if(m_lvecvec)
	{
		for( int d = 0; d < m_depth; d++ ) if(m_lvecvec[d]) mxFree(m_lvecvec[d]);
        mxFree(m_lvecvec);
	}
	if(m_avecvec)
	{
		for( int d = 0; d < m_depth; d++ ) if(m_avecvec[d]) mxFree(m_avecvec[d]);
		mxFree(m_avecvec);
	}
	if(m_bvecvec)
	{
		for( int d = 0; d < m_depth; d++ ) if(m_bvecvec[d]) mxFree(m_bvecvec[d]);
		mxFree(m_bvecvec);
	}
}
	...
	double*&					bvec)
{
	...
	lvec = (double*) mxMalloc(sz * sizeof(double));
	avec = (double*) mxMalloc(sz * sizeof(double));
	bvec = (double*) mxMalloc(sz * sizeof(double));
...
//	const int dy8[8] = { 0, -1, -1, -1, 0, 1, 1,  1};
	...
	int label(0);
	int* xvec = (int*) mxMalloc(sz * sizeof(int));
	int* yvec = (int*) mxMalloc(sz * sizeof(int));
	int oindex(0);
	...
	const int&					STEP)
{
	...
	int adjlabel(0);//adjacent label
        int* xvec = (int*) mxMalloc(SUPSZ*10 * sizeof(int));//a large enough size
        int* yvec = (int*) mxMalloc(SUPSZ*10 * sizeof(int));//a large enough size
        int* zvec = (int*) mxMalloc(SUPSZ*10 * sizeof(int));//a large enough size
	//------------------
	// memory allocation
	//------------------
	int** nlabels = (int**) mxMalloc(depth * sizeof(int*));
	{for( int d = 0; d < depth; d++ )
	{
		nlabels[d] = (int*) mxMalloc(sz * sizeof(int));
    ...
    const double&               compactness)
{
	...
	//klabels.resize( sz, -1 );
	//--------------------------------------------------
	klabels = (int*) mxMalloc(sz * sizeof(int));
    ...
    {
        m_lvec = (double*) mxMalloc(sz * sizeof(double)); m_avec = (double*) mxMalloc(sz * sizeof(double)); m_bvec = (double*) mxMalloc(sz * sizeof(double));
        for( int i = 0; i < sz; i++ )
        {
    ...
    const double&               compactness)
{
	...
	m_lvecvec = (double**) mxMalloc(depth * sizeof(double*));
	m_avecvec = (double**) mxMalloc(depth * sizeof(double*));
	m_bvecvec = (double**) mxMalloc(depth * sizeof(double*));
	for( int d = 0; d < depth; d++ )
	{
                //klabels[d] = new int[sz];
		m_lvecvec[d] = (double*) mxMalloc(depth * sizeof(sz));
		m_avecvec[d] = (double*) mxMalloc(depth * sizeof(sz));
		m_bvecvec[d] = (double*) mxMalloc(depth * sizeof(sz));
		for( int s = 0; s < sz; s++ )
		{
			klabels[d][s] = -1;
		}
