...
SLICO::~SLICO()
{
	...
	if(m_lvecvec)
	{
		for( int d = 0; d < m_depth; d++ ) mxFree(m_lvecvec[d]);
        mxFree(m_lvecvec);
	}
	if(m_avecvec)
	{
		for( int d = 0; d < m_depth; d++ ) mxFree(m_avecvec[d]);
		mxFree(m_avecvec);
	}
	if(m_bvecvec)
	{
		for( int d = 0; d < m_depth; d++ ) mxFree(m_bvecvec[d]);
		mxFree(m_bvecvec);
	}
}
...
//===========================================================================
/*void SLICO::SaveSuperpixelLabels(
	const int*					labels,
	const int&					width,
	const int&					height,
